...
$(document).ready(function () {
  ...
  $('#searchBtn').on('click', function (event) {
    ...
    graphData(
      'global-chart',
      [TotalConfirmed, GlobalTotalConfirmed, NewConfirmed, GlobalNewConfirmed],
      [
        'Country Total Confirmed',
        'Global Total Confirmed',
        'Country New Confirmed',
  ...
  function getCountryPopulation(query) {
    var settings = {
      ...
      crossDomain: true,
      url:
        'https://world-geo-data.p.rapidapi.com/countries/' +
        query +
        '?language=en%252Cru%252Czh%252Ces%252Car%252Cfr%252Cfa%252Cja%252Cpl%252Cit%252Cpt%252Cde&format=json',
      method: 'GET',
      headers: {
    ...
    $.ajax(settings).done(function (response) {
      ...
      for (var i = 0; i < 4; i++) {
        ...
        $('.card .card-body .card-title')
          .eq(i)
          .html(response.articles[randNum + i].title);
        $('.card .card-body .card-text')
          .eq(i)
          .html(truncate(response.articles[randNum + i].summary, 50) + '...');
        $('.card .card-body .btn-primary')
          .eq(i)
          .attr('href', response.articles[randNum + i].link);
      }
    });
  }
